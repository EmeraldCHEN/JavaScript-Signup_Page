function registCheckXXXXXXXXXXX(){
    ...
    if(textValue.length != 11){
        document.getElementById('phonemsg').innerHTML = 'Please fill in 11 digits.'
    }            
    else{
...
function phoneOnblur(){
   var textValue = document.getElementsByName('phone')[0].value;       // ducument ...... SPELLING!!!!!
                    
   if(textValue.length != 11){
        document.getElementById('phonemsg').innerHTML = 'Must be 11 digits';
   }
   else{
        for(var i = 0; i < textValue.length; i++){
            if(textValue.substr(i,1)<"0" || textValue.substr(i,1) > "9"){
...
function passwordDoubleCheckOnblur(){
    ...
    // wITHOUT the above, ERROR: textValue1 is not defined.
